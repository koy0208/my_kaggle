...
class Submission:
    ...
    def create_submission(cls, run_name):
        ...
        pred = Util.load('../model/pred/{}-test.pkl'.format(run_name))
        for i in range(pred.shape[1]):
            submission['Class_{}'.format(i + 1)] = pred[:, i]
        submission.to_csv('../submission/{}.csv'.format(run_name), index=False)
